...
if($result && mysqli_num_rows($result)>0){
	...
	if($table == 'meetingattendance' && $studentID !== null) // exception for meetingattendance table
	{
		$query = "UPDATE `$table` SET `$field`='$value' WHERE `$table`.`".$table."ID` = $myID"."AND `$table`.`studentID` = $studentID";
	}
	else
	{
